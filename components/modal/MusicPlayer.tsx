...
export default function MusicPlayer({ autoPlay = true }: { autoPlay?: boolean }) {
  ...
  // Reset visual state saat track berubah
  useEffect(() => {
    if (!audioRef.current) return;
    setCurrentTime(0);
    setDuration(0);
    audioRef.current.currentTime = 0;
  }, [currentTrack?.path, setCurrentTime, setDuration]);
  ...
  return (
    <div className="w-full h-full text-white flex flex-col justify-end items-center">
      ...
      {showPlaylist && (
        <div
          className="bg-white dark:bg-slate-900 overflow-y-auto border-b border-slate-200 dark:border-slate-700 w-full max-h-60 shadow-lg"
      ...
      {/* CONTROLS */}
      <div
        className="bg-white dark:bg-slate-900 w-full p-5 border-t border-slate-200 dark:border-slate-800"
        onClick={(e) => e.stopPropagation()}
      >
        <p className="text-black dark:text-white font-medium truncate text-sm mb-2">
          {currentTrack.name}
        </p>
